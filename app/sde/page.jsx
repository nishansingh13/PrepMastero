...
export default function SDESheet() {
  ...
  const getSDEQuestions = async() => {
    try {
      setLoading(true);
      const res = await axios.get("/api/sde/getData");
      setProblems(res.data.data);
      setFilteredProblems(res.data.data);
    } catch (error) {
      console.error("Error fetching SDE problems:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const updateProblemStatus = async (problemId, newStatus) => {
    ...
    try {
      // Update in backend
      await axios.put(`/api/sde/updateStatus/${problemId}`, { status: newStatus });
  ...
  const updateProblemSolved = async (problemId, isSolved) => {
    ...
    try {
      // Update in backend
      await axios.put(`/api/sde/updateStatus/${problemId}`, { solved: isSolved });
    } catch (error) {
  ...
  const filterProblems = (problemList, search, difficulty, status) => {
    ...
    if (search) {
      filtered = filtered.filter(problem =>
        (problem?.Name && problem.Name.toLowerCase().includes(search.toLowerCase()))
      );
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-gray-600">Loading SDE problems...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Striver's SDE Sheet</h1>
          <p className="text-lg text-gray-600">Software Development Engineer Interview Preparation</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-pink-500">
            ...
            <p className="text-3xl font-bold text-pink-600">{stats.percentage}%</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div 
              className="bg-gradient-to-r from-purple-400 to-pink-600 h-3 rounded-full transition-all duration-300"
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 border-b">
            <h2 className="text-xl font-semibold text-gray-800">
              SDE Problems ({filteredProblems.length})
            </h2>
          </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <select
                      ...
                      className={`px-3 py-1 border rounded-lg text-sm font-medium focus:ring-2 focus:ring-purple-500 ${getStatusColor(problem?.status || "Not Solved")}`}
                    >
                      <option value="Not Solved">Not Solved</option>
                      <option value="easily done">Easily Done</option>
                      <option value="need revisit">Need Revisit</option>
                    ...
                    {problem?.Link && (
                      <a
                        ...
                        className="inline-flex items-center px-3 py-1 border border-purple-300 text-sm font-medium rounded-lg text-purple-700 bg-purple-50 hover:bg-purple-100 transition-colors"
                      >
